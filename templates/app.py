...
def kill_app():
    ...
    def _shutdown():
        import time
        time.sleep(0.5)
        # copy DB back to scripts3 so Termux can retrieve it
        scripts3_db = Path("/storage/emulated/0/Android/data/org.qpython.qpy3/scripts3/blog7/blog7.db")
        try:
            shutil.copy2(str(DB_PATH), str(scripts3_db))
        except Exception:
            pass
    ...
    return "<html><body><script>history.go(-(history.length-1));setTimeout(function(){window.location.replace('about:blank');},400);</script>Stopping...</body></html>"
